...
@ T_CALCULO = `find . -type d -iname '*calculo*' | wc -l`
@ T_FASTLOC1 = `find . -type d -iname '*fast_loc_I_*' | wc -l`
@ T_FASTLOC2 = `find . -type d -iname '*fast_loc_II_*' | wc -l`
...
@ CALCULO_LAST = `find . -type d -iname '*calculo*' | cut -f 2 -d '_' | cut -f 1 -d '_' | sort -n | tail -n1`
@ FASTLOC1_LAST = `find . -type d -iname '*fast_loc_I_*' | cut -f 2 -d '_' | cut -f 1 -d '_' | sort -n | tail -n1`
@ FASTLOC2_LAST = `find . -type d -iname '*fast_loc_II_*' | cut -f 2 -d '_' | cut -f 1 -d '_' | sort -n | tail -n1`
...
printf "FASTLOC1 possui $T_FASTLOC1 imagem(ns) e a última realizada foi:	%03d\n" $FASTLOC1_LAST
printf "FASTLOC2 possui $T_FASTLOC2 imagem(ns) e a última realizada foi:	%03d\n" $FASTLOC2_LAST
printf "CALCULO possui $T_CALCULO imagem(ns) e a última realizada foi:	%03d\n" $CALCULO_LAST
...
foreach serie_localizador ( `find -maxdepth 1 ! -iname '*PSEUDO*' ! -iname '*CALCULO*' ! -iname '*FAST_LOC*' ! -iname '*DTI*' ! -iname '*RST*' ! -iname '*3D*' ! -iname '*dicom*' -type d | sed 's:^./::' | cut -f 2 -d '_' | cut -f 1 -d '_' | tail -n +2` )
...
if ( $T_CALCULO > 1 ) then
	echo "Calculo tem mais de uma série."
	echo "Removendo as anteriores..."
	foreach i ( `find . -type d -iname '*CALCULO*' | cut -f 2 -d '_' | cut -f 1 -d '_' | sort -n | sed '$d'` )
  		find . -maxdepth 1 -type d -iname "*${i}*calculo*" -exec rm -rv {} \;
	end 
endif

if ( $T_FASTLOC1 > 1 ) then
	echo "Fastloc1 tem mais de uma série."
	echo "Removendo as anteriores..."
	foreach i ( `find . -type d -iname '*FAST_LOC_I_*' | cut -f 2 -d '_' | cut -f 1 -d '_' | sort -n | sed '$d'` )
  		find . -maxdepth 1 -type d -iname "*${i}*fast_loc_I_*" -exec rm -rv {} \;
	end 
endif

if ( $T_FASTLOC2 > 1 ) then
	echo "Fastloc2 tem mais de uma série."
	echo "Removendo as anteriores..."
	foreach i ( `find . -type d -iname '*FAST_LOC_II_*' | cut -f 2 -d '_' | cut -f 1 -d '_' | sort -n | sed '$d'` )
  		find . -maxdepth 1 -type d -iname "*${i}*fast_loc_II_*" -exec rm -rv {} \;
	end 
endif
...
set serie_number = `echo $x | grep -oP "[a-z][0-9][0-9][0-9][a-z]" | cut -f 2 -d 's' | cut -f 1 -d 'a'`
	if ( $x =~ *".bval" ) then
...
set CALCULO = `find . -maxdepth 1 -iname "dicom" -prune -o -print | egrep -io "(CALCULO)"`
set FASTLOC1 = `find . -maxdepth 1 -iname "dicom" -prune -o -print | egrep -io "(FAST)[[:punct:]](LOC)[[:punct:]](I)[[:punct:]]"`
set FASTLOC2 = `find . -maxdepth 1 -iname "dicom" -prune -o -print | egrep -io "(FAST)[[:punct:]](LOC)[[:punct:]](II)[[:punct:]]"`
...
foreach y ( `ls -d */ | grep -vwE "(dicom)" | sed 's:/$::'` )
	...
	if ( $y =~ *"$CALCULO"* && $CALCULO_LAST > 0 ) then
		mv -v ${y}/ CALCULO/
		cd CALCULO
		mv *.nii.gz ${study}${subj}.CALCULO.nii.gz
		cd ..
	endif

	if ( $y =~ *"$FASTLOC1"* && $FASTLOC1_LAST > 0 ) then
		mv -v ${y}/ FASTLOC1/
		cd FASTLOC1
		mv *.nii.gz ${study}${subj}.FASTLOC1.nii.gz
		cd ..
	endif

	if ( $y =~ *"$FASTLOC2"* && $FASTLOC2_LAST > 0 ) then
		mv -v ${y}/ FASTLOC2/
		cd FASTLOC2
		mv *.nii.gz ${study}${subj}.FASTLOC2.nii.gz
		cd ..
	endif
